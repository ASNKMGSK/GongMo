...
async def deliberate(
    *,
    ...
    # ★ 2026-04-30: 판사는 RAG/HITL 미사용 (사용자 정책). HITL 은 페르소나 토론 단계에서만 주입.
    ...
    return {
        ...
        # ★ 2026-04-30: 판사는 RAG/HITL 미사용 — human_cases_meta 빈 배열 (스키마 호환).
        "human_cases_meta": [],
    }
...
def _build_post_debate_user_message(
    *,
    ...
    ★ 2026-04-30: 판사는 RAG/HITL 미사용 — human_cases 파라미터 제거.
    """
    allowed = ALLOWED_STEPS.get(item_number, [])
...
async def deliberate_post_debate(
    *,
    ...
    # ★ 2026-04-30: 판사는 RAG/HITL 미사용. HITL 은 페르소나 토론 단계에서 이미 주입됨.
    ...
    try:
        ...
        return _post_debate_fallback(
            ...
            failure_reason=failure, retrieved_human_cases_count=0,
        )
    ...
    return {
        ...
        "judge_used": True,
        "persona_label_map": label_map,
        # ★ 2026-04-30: 판사는 RAG/HITL 미사용. HITL 사례는 페르소나 토론 단계에서 주입.
        "retrieved_human_cases_count": 0,
        "human_cases_meta": [],
    }
        ...
        summaries.append(
            {
                ...
                "transcript_excerpt": h.get("transcript_excerpt") or "",
                "human_note": h.get("human_note") or "",
                "ai_judgment": h.get("ai_judgment") or "",
                # ★ 2026-05-07: golden_set md "## 파싱 원문" — Layer1 평가그룹별 분할 문맥.
                # segment_text(=근거) 와 별개. 프론트 카드에 별도 섹션으로 노출.
                # 2026-05-08: 600자 cap 제거 — 프론트 토글로 처리.
                "parsed_text": h.get("parsed_text") or "",
                # ★ 2026-04-30: "이 케이스가 현재 평가 중인 원문 자체의 골든셋인지" 표시.
                # run_debate.py 가 retrieve 후 is_self_match=True 로 마킹.
                "is_self_match": bool(h.get("is_self_match", False)),
                # ★ 2026-04-30: 골든셋 RAG 출처 — 현재는 HITL 만 있지만 향후 외부 큐레이션 데이터 등
                # 다른 source 가 추가될 수 있어 source 필드로 명시. 프론트가 출처별 배지 렌더.
                "source": str(h.get("source") or "hitl"),
                # ★ 2026-05-07: golden_set example_id (qa-golden-set 인덱스 식별자) 보존.
