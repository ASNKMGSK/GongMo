...
def parse_llm_json(raw_text: Any) -> dict:
    ...
    # 6차: json-repair 라이브러리 폴백 — unterminated string / 이스케이프 누락 / 잘못된 따옴표 등
    # 자체 5단계 복구가 못 잡는 케이스 (예: 문자열 안에 raw newline / 짝 안 맞는 quote) 처리.
    try:
        ...
        if isinstance(result, dict):
            logger.warning("parse_llm_json: recovered via json-repair lib (original error: %s)", first_err)
            return result
        # 리스트로 복구되면 dict 로 감쌈 (Sub Agent 가 dict 기대)
        if isinstance(result, list):
            logger.warning("parse_llm_json: json-repair returned list, wrapping as {'items': [...]}")
            return {"items": result}
    except Exception as repair_err:
        logger.info("parse_llm_json: json-repair fallback also failed: %s", repair_err)
