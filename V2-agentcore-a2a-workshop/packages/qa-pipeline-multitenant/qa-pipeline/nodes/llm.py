...
#   - "bedrock"  : Bedrock Sonnet 4.5 (기본값, 현재 비교 테스트 중)
...
# Bedrock 모델 ID (Sonnet 4.5 inference profile)
...
# 동시 요청 제한: 단일 인스턴스에 너무 많은 요청이 몰리지 않도록 세마포어로 제어
# 8개 에이전트가 각 2~3회 LLM 호출 → 최대 ~18개 동시 요청 발생
# GPU 1장 기준 3~4개 동시 처리가 적정 (vLLM continuous batching)
MAX_CONCURRENT_REQUESTS = app_config.sagemaker.max_concurrent
_semaphore: asyncio.Semaphore | None = None


def _get_semaphore() -> asyncio.Semaphore:
    """비동기 세마포어 싱글턴 (이벤트 루프 내에서 생성)."""
    global _semaphore
    if _semaphore is None:
        _semaphore = asyncio.Semaphore(MAX_CONCURRENT_REQUESTS)
        logger.info("SageMaker concurrency limiter: max %d concurrent requests", MAX_CONCURRENT_REQUESTS)
    return _semaphore
...
def get_chat_model(
    *,
    ...
    if chosen == "bedrock":
        # boto3 기본값 (read_timeout=60, retries=legacy=5회) 로는 긴 리포트 생성이
        # read timeout → retry 루프로 300s+ 걸림. read_timeout 180s + max_attempts=2 로 조정.
        boto_cfg = BotoConfig(read_timeout=180, connect_timeout=10, retries={"max_attempts": 2, "mode": "standard"})
...
async def ainvoke_llm(
    ...
    *,
    ...
    if cached is not None:
        logger.debug("ainvoke_llm cache hit: key=%s", cache_key[:12])
        return cached
    llm = get_chat_model(
...
async def invoke_and_parse(llm: BaseChatModel, messages: list) -> dict:
    ...
    cached = _get_cached_response(cache_key)
    if cached is not None:
        logger.debug("invoke_and_parse cache hit: key=%s", cache_key[:12])
        return cached
    response = await llm.ainvoke(messages)
