"""
CDK Stack for the Orchestrator Agent — AgentCore Runtime.

The Orchestrator is the main entry point for user requests. It classifies
user intent and delegates to specialised agents (MCP Data, RAG, Summary)
...
class OrchestratorAgentStack(Stack):
    """CDK Stack for Orchestrator Agent using AgentCore Runtime."""

    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        super().__init__(scope, construct_id, **kwargs)

        agentcore_context = self.node.try_get_context("orchestrator-agent-agentcore") or {}
        self.tool_name = agentcore_context.get("tool-name", "orchestrator_agent")
    ...
    def _create_agentcore_role(self) -> iam.Role:
        ...
        self.agentcore_policy = iam.Policy(
            ...
            statements=[
                ...
                iam.PolicyStatement(sid="GetAgentAccessToken", actions=["bedrock-agentcore:GetWorkloadAccessToken", "bedrock-agentcore:GetWorkloadAccessTokenForJWT", "bedrock-agentcore:GetWorkloadAccessTokenForUserId"], resources=[f"arn:aws:bedrock-agentcore:{self.region}:{self.account}:workload-identity-directory/default*", f"arn:aws:bedrock-agentcore:{self.region}:{self.account}:workload-identity-directory/default/workload-identity/{self.tool_name}-*"]),
                ...
                iam.PolicyStatement(sid="BedrockAgentCoreMemoryAccess", actions=["bedrock-agentcore:CreateMemory", "bedrock-agentcore:GetMemory", "bedrock-agentcore:UpdateMemory", "bedrock-agentcore:DeleteMemory", "bedrock-agentcore:ListMemories", "bedrock-agentcore:CreateEvent", "bedrock-agentcore:GetEvent", "bedrock-agentcore:ListEvents", "bedrock-agentcore:RetrieveMemoryRecords"], resources=["*"]),
                ...
                # A2A: invoke other agent runtimes
                iam.PolicyStatement(sid="A2AInvokeAccess", actions=["bedrock-agentcore:InvokeRuntime"], resources=[f"arn:aws:bedrock-agentcore:{self.region}:{self.account}:runtime/*"]),
            ],
        )
    ...
    def _create_memory(self) -> agentcore.Memory:
        return agentcore.Memory(
            self, "OrchestratorMemory",
            memory_name=f"{self.tool_name}_memory",
            description="Memory for orchestrator agent conversations",
    ...
    def _create_secret_update_resource(self, username, password):
        update_role = iam.Role(
            ...
            inline_policies={"SecretsAndCognitoAccess": iam.PolicyDocument(statements=[
        ...
        fn = lambda_.Function(
            self, "UpdateSecretFunction",
            runtime=lambda_.Runtime.PYTHON_3_12, handler="index.handler", role=update_role,
...
def handler(event, context):
    if event['RequestType'] in ['Create', 'Update']:
        ...
        secret_value = json.dumps({'user_pool_id': p['UserPoolId'], 'client_id': p['ClientId'], 'username': p['Username'], 'password': p['Password'], 'discovery_url': p['DiscoveryUrl']})
        try:
            secrets.update_secret(SecretId=p['SecretName'], SecretString=secret_value)
        except secrets.exceptions.ResourceNotFoundException:
            secrets.create_secret(Name=p['SecretName'], SecretString=secret_value)
...
"""),
        ...
        resource.node.add_dependency(self.user_pool_client)
    ...
    def _create_agentcore_runtime(self) -> agentcore.Runtime:
        agent_path = Path(__file__).parent.parent.parent.parent / "agentcore-agents" / "orchestrator-agent"
        runtime = agentcore.Runtime(
            self, "OrchestratorAgentRuntime",
    ...
    def _create_ssm_parameters(self):
        ...
        ssm.StringParameter(self, "RoleNameParam", parameter_name=f"/{self.tool_name}/runtime/agent_role_name", string_value=self.agentcore_role.role_name)
        ...
        ssm.StringParameter(self, "MemoryIdParam", parameter_name=f"/{self.tool_name}/memory/memory_id", string_value=self.memory.memory_id)
    ...
    def _create_outputs(self):
        CfnOutput(self, "AgentCoreRoleArn", value=self.agentcore_role.role_arn)
