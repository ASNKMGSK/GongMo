"""
CDK Stack for the Weather MCP Server — AgentCore Runtime (MCP protocol).

The Weather MCP Server provides weather tools (current weather, forecasts,
alerts) via the MCP protocol. It is registered as a target on the
AgentCore Gateway so the Orchestrator can invoke weather tools through
the Gateway.
"""
...
class WeatherMcpStack(Stack):
    """CDK Stack for Weather MCP Server using AgentCore Runtime (MCP protocol)."""

    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        super().__init__(scope, construct_id, **kwargs)

        agentcore_context = self.node.try_get_context("weather-mcp-agentcore") or {}
        self.tool_name = agentcore_context.get("tool-name", "a2a_weather_mcp")
...
'''))
    ...
    def _create_agentcore_runtime(self) -> agentcore.Runtime:
        mcp_server_path = Path(__file__).parent.parent.parent.parent / "agentcore-mcp-servers" / "weather-mcp"
        runtime = agentcore.Runtime(
            self, "WeatherMCPRuntime",
            runtime_name=self.tool_name,
            agent_runtime_artifact=agentcore.AgentRuntimeArtifact.from_asset(str(mcp_server_path), platform=ecr_assets.Platform.LINUX_ARM64),
