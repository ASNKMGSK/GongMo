"""
CDK Stack for the QA Pipeline Agent — AgentCore Runtime (HTTP protocol).

The QA Pipeline runs a multi-stage LLM-as-a-judge evaluation pipeline
against a SageMaker-hosted Qwen3-8B endpoint. Deployed as an HTTP
(SSE-streaming) FastAPI server behind AgentCore Runtime, authenticated
via Cognito JWT (user access tokens) and OAuth2 client_credentials for
downstream Gateway integration.
"""
...
class QaPipelineStack(Stack):
    """CDK Stack for QA Pipeline Agent using AgentCore Runtime (HTTP protocol, SageMaker backend)."""

    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        super().__init__(scope, construct_id, **kwargs)

        agentcore_context = self.node.try_get_context("qa-pipeline-agentcore") or {}
        self.tool_name = agentcore_context.get("tool-name", "qa_pipeline")
        self.sagemaker_endpoint_name = agentcore_context.get("sagemaker-endpoint", "qwen3-8b-vllm")
        ...
        self.agentcore_policy = iam.Policy(
            ...
            statements=[
                ...
                iam.PolicyStatement(
                    ...
                    resources=[
                        f"arn:aws:sagemaker:{self.region}:{self.account}:endpoint/{self.sagemaker_endpoint_name}",
                    ],
                ),
                iam.PolicyStatement(
                    sid="ECRImageAccess",
                ...
                iam.PolicyStatement(
                    ...
                    resources=[f"arn:aws:bedrock-agentcore:{self.region}:{self.account}:workload-identity-directory/default*"],
                ),
                iam.PolicyStatement(
    ...
    # ------------------------------------------------------------------
    # Cognito (OAuth2 client_credentials for Gateway; JWT for Runtime auth)
    # ------------------------------------------------------------------
    ...
    def _create_cognito(self):
        ...
        resource_server = user_pool.add_resource_server(
            ...
            scopes=[cognito.ResourceServerScope(scope_name="invoke", scope_description="Invoke QA Pipeline")],
        )
        user_pool_client = user_pool.add_client(
            ...
            o_auth=cognito.OAuthSettings(
                ...
                scopes=[cognito.OAuthScope.resource_server(
                    resource_server,
                    cognito.ResourceServerScope(scope_name="invoke", scope_description="Invoke QA Pipeline"),
                )],
            ),
        )
    ...
    def _store_cognito_credentials(self):
        cr_role = iam.Role(
            ...
            inline_policies={"SecretsAccess": iam.PolicyDocument(statements=[
                iam.PolicyStatement(
                    actions=["secretsmanager:CreateSecret", "secretsmanager:UpdateSecret", "secretsmanager:DeleteSecret", "secretsmanager:DescribeSecret"],
                    resources=[f"arn:aws:secretsmanager:{self.region}:{self.account}:secret:{self.tool_name}/cognito/*"],
                ),
                iam.PolicyStatement(actions=["cognito-idp:DescribeUserPoolClient"], resources=[self.user_pool.user_pool_arn]),
            ])},
        )
        fn = lambda_.Function(
            self, "StoreCredsFunction",
            runtime=lambda_.Runtime.PYTHON_3_12, handler="index.handler",
            timeout=Duration.minutes(2), role=cr_role,
...
'''),
    ...
    def _create_agentcore_runtime(self) -> agentcore.Runtime:
        agent_path = Path(__file__).parent.parent.parent.parent / "agentcore-agents" / "qa-pipeline"
        runtime = agentcore.Runtime(
            self, "QaPipelineRuntime",
    ...
    def _create_endpoint_url(self) -> str:
        cr_role = iam.Role(
            self, "EndpointUrlRole",
            assumed_by=iam.ServicePrincipal("lambda.amazonaws.com"),
            managed_policies=[iam.ManagedPolicy.from_aws_managed_policy_name("service-role/AWSLambdaBasicExecutionRole")],
        )
        cr_function = lambda_.Function(
            self, "EndpointUrlFunction",
            runtime=lambda_.Runtime.PYTHON_3_12, handler="index.handler",
            timeout=Duration.seconds(30), role=cr_role,
            code=lambda_.Code.from_inline('''
import urllib.parse
def handler(event, context):
...
'''),
    ...
    def _apply_cdk_nag_suppressions(self):
        NagSuppressions.add_resource_suppressions(self.agentcore_policy, [{"id": "AwsSolutions-IAM5", "reason": "Wildcard permissions required for Bedrock, ECR, X-Ray, SSM, SecretsManager."}])
