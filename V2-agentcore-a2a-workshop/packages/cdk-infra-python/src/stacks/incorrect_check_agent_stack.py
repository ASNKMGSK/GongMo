...
"""
CDK Stack for the Incorrect Check Agent — AgentCore Runtime (MCP protocol).

Evaluates process compliance (프로세스 준수) in consultation transcripts.
Deployed as an MCP server so it can be registered as a target on the AgentCore Gateway.
"""
...
class IncorrectCheckAgentStack(Stack):
    """CDK Stack for Incorrect Check Agent using AgentCore Runtime (MCP protocol)."""

    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        super().__init__(scope, construct_id, **kwargs)

        agentcore_context = self.node.try_get_context("incorrect-check-agent-agentcore") or {}
        self.tool_name = agentcore_context.get("tool-name", "a2a_incorrect_check_agent")
...
'''))
    ...
    def _create_agentcore_runtime(self) -> agentcore.Runtime:
        agent_path = Path(__file__).parent.parent.parent.parent / "agentcore-agents" / "incorrect-check-agent"
        runtime = agentcore.Runtime(
            self, "IncorrectCheckAgentRuntime",
