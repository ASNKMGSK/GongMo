"""
CDK Stack for the RAG Agent — AgentCore Runtime (MCP protocol).

The RAG Agent searches the OpenSearch Serverless knowledge base for
relevant document chunks and returns results. Deployed as an MCP server
so it can be registered as a target on the AgentCore Gateway.
"""
...
class RagAgentStack(Stack):
    """CDK Stack for RAG Agent using AgentCore Runtime (MCP protocol)."""

    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        super().__init__(scope, construct_id, **kwargs)

        agentcore_context = self.node.try_get_context("rag-agent-agentcore") or {}
        self.tool_name = agentcore_context.get("tool-name", "a2a_rag_agent")
    ...
    def _create_agentcore_role(self) -> iam.Role:
        ...
        self.agentcore_policy = iam.Policy(
            ...
            statements=[
                ...
                iam.PolicyStatement(sid="OpenSearchAccess", actions=["aoss:APIAccessAll"], resources=[f"arn:aws:aoss:{self.region}:{self.account}:collection/*"]),
                iam.PolicyStatement(sid="S3ReadAccess", actions=["s3:GetObject", "s3:ListBucket"], resources=["*"]),
                iam.PolicyStatement(sid="DynamoDBAccess", actions=["dynamodb:GetItem", "dynamodb:Query", "dynamodb:Scan"], resources=["*"]),
            ],
        )
...
'''))
    ...
    def _create_agentcore_runtime(self) -> agentcore.Runtime:
        agent_path = Path(__file__).parent.parent.parent.parent / "agentcore-agents" / "rag-agent"
        runtime = agentcore.Runtime(
            self, "RagAgentRuntime",
...
'''))
    ...
    def _apply_cdk_nag_suppressions(self):
        NagSuppressions.add_resource_suppressions(self.agentcore_policy, [{"id": "AwsSolutions-IAM5", "reason": "Wildcard permissions required for Bedrock, ECR, OpenSearch, X-Ray."}])
