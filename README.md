...
| 위치 | 설명 |
|---|---|
| `V2-agentcore-a2a-workshop/` | **메인 모노레포** — 백엔드 (qa-pipeline) + 프론트 (chatbot-ui-next) + CDK 인프라 + 배포 스크립트 |
| `AI_QA_Agent_Design_Document_v2.pdf` | 시스템 디자인 문서 (아키텍처, 평가 정책, 데이터 흐름) |
| `STT_기반_통합_상담평가표_v2.xlsx` | 21항목 QA 평가표 원본 |
| `_gen_samples.py`, `_gen_samples_v2.py` | 점수대별 샘플 데이터 생성기 (4 tier × 5 샘플) |
| `CLAUDE.md` | Claude Code 작업 가이드 (프로젝트 지시문) |
...
### Backend (포트 8081)
```bash
cd V2-agentcore-a2a-workshop/packages/agentcore-agents/qa-pipeline
...
### Frontend (포트 3000)
```bash
cd V2-agentcore-a2a-workshop/packages/chatbot-ui-next
pnpm install
pnpm dev
```
...
`QAState.plan.skip_phase_c_and_reporting=True` 주입 시 phase_b2 후 즉시 `__end__` (report/validation/consistency 생략, 약 17초/샘플 절감).
...
| **PII 정책** | strict (카드사 — 주민/카드/계좌 zero-tolerance) | standard (의류몰) |
...
3-페르소나 round_robin 그룹채팅으로 의견 충돌 합의:
...
- **HITL Step 0**:
  - 강한 신호 (cos≥0.7) → 채택 (규칙 A)
  - 약한 신호 (0.5≤cos<0.7) → 참조 (규칙 B)
  - 사례 없음 → 일반 평가 (규칙 C)
...
상세: `V2-agentcore-a2a-workshop/CLAUDE.md`

---

## 📚 참고 문서

- `AI_QA_Agent_Design_Document_v2.pdf` — 시스템 디자인 (PDF, 182KB)
- `STT_기반_통합_상담평가표_v2.xlsx` — 21항목 QA 평가표
- `V2-agentcore-a2a-workshop/CLAUDE.md` — 코드베이스 작업 가이드
- `V2-agentcore-a2a-workshop/DEVELOPER_GUIDE.md` — 개발자 온보딩
- `V2-agentcore-a2a-workshop/packages/chatbot-ui-next/DEPLOYMENT.md` — 프론트 배포 상세
- `V2-agentcore-a2a-workshop/packages/agentcore-agents/qa-pipeline/deploy/README.md` — EC2 배포 상세

---

## 📜 License

Apache License 2.0 — see `V2-agentcore-a2a-workshop/LICENSE` (Amazon.com, Inc. or its affiliates).
